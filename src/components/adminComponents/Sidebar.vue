...
<template>
  <nav
    :class="[
      'bg-[#F4F8FA] h-full rounded-6 flex flex-col gap-10 overflow-hidden dark:bg-primary-900',
      settingsStore.settings.isSidebarCollapsed
        ? 'w-26 items-center'
        : 'w-90 items-start',
    ]"
  >
      ...
      >
        <template #item="{ item }">
          <router-link
            ...
            :to="{ name: item.route }"
            :class="[
              'w-full flex items-center gap-4 p-4 rounded-3 hover:bg-[#E5E7EB] dark:hover:bg-primary-800',
              { 'bg-[#E5E7EB] dark:bg-primary-800': route.name === item.route },
            ]"
          >
            <i :class="[item.icon, 'text-5']"></i>
            <span v-if="!settingsStore.settings.isSidebarCollapsed">{{
              item.label
            }}</span>
          </router-link>
          <div
            ...
            :class="[
              'w-full flex items-center gap-4 p-4 rounded-3 hover:bg-[#E5E7EB] dark:hover:bg-primary-800',
              {
                'bg-[#E5E7EB] dark:bg-primary-800':
                  item.items &&
                  (item.items as NavItem[]).some(
                    (child: NavItem) => route.name === child.route,
                  ),
              },
            ]"
            v-tooltip.right="
          ...
          >
            ...
            <i
              v-if="!settingsStore.settings.isSidebarCollapsed"
              class="i-hugeicons-arrow-down-01 ms-auto"
            ></i>
          </div>
        </template>
      </PanelMenu>
