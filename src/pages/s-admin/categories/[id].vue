...
const UserSchema = toTypedSchema(
    zod.object({
        name_en: zod.string().min(3).max(255).regex(/^[A-Za-z0-9\s\-\_\.]+$/, t('must_be_english')),
        name_ar: zod.string().min(3).max(255).regex(/^[\u0600-\u06FF\s0-9\-\_\.]+$/, t('must_be_arabic')),
...
watchEffect(() => {
    if (name_en.value) {
        slug.value = name_en.value.toLowerCase().replace(/ /g, '-').replace(/[^\u0000-\u007F]/g, (char) => arabicToEnglishMap[char] || char);
    }
...
const { mutateAsync } = useMutation({
    ...
    onSuccess: (res) => {
        ...
        appStore.toast.add({ severity: 'info', summary: 'Success', detail: 'Category saved successfully', life: 3000 });
    },
    onError: (error) => {
        ...
        <form @submit="onSubmit" class="w-full h-full flex flex-col gap-6 overflow-hidden">
            ...
            <div class="grid grid-cols-2 gap-6">
                <div class="flex flex-col gap-1">
                    ...
                    <InputText v-model="name_ar" :class="{ 'p-invalid': errors.name_ar }" />
                    <span v-if="errors.name_ar" class="text-red-500">{{ errors.name_ar }}</span>
                </div>
                <div class="flex flex-col gap-1">
                    <label class="text-[#4b465c82] text-3.5 font-semibold" for="fname">{{ $t('name_en') }}</label>
                    <InputText v-model="name_en" :class="{ 'p-invalid': errors.name_en }" />
                    <span v-if="errors.name_en" class="text-red-500">{{ errors.name_en }}</span>
                </div>
                <div class="flex flex-col gap-1">
                ...
                <div class="flex flex-col gap-1">
                    <label class="text-[#4b465c82] text-3.5 font-semibold" for="fname">{{ $t('category.parent_category')
                        }}</label>
                    <Select v-model="parent_id" :options="categories" optionLabel="name" optionValue="id"
                ...
                <div class="flex flex-col gap-1 col-span-2">
                    ...
                    <FileUpload ref="fileupload" :fileLimit="1" name="demo[]" accept="image/*" :maxFileSize="1000000"
                        customUpload :class="{ 'p-invalid': errors.parent_id }" />
                    <span v-if="errors.parent_id" class="text-red-500">{{ errors.parent_id }}</span>
                </div>
