  ...
  if (data.value) {
    ...
    if (permissions.value) {
      if (allSelected.value) {
        ...
        selectedPermissions.value = permissions.value.map((permission: any) =>
          data.value.permissions.some((p: any) => p.id === permission.id),
        );
      }
    }
  }
        ...
        permission_ids: permissions.value
          .filter(
            (permission: any, index: number) =>
              selectedPermissions.value?.[index],
          )
    ...
    >
      <div class="border rounded-4 w-full p-4 flex gap-6">
        <div class="flex flex-col gap-1 w-full">
          <label class="text-[#4b465c82] text-3.5 font-semibold" for="name"
            >Name</label
          >
        ...
        <div class="flex flex-col gap-1 w-full">
          ...
          />
          <span v-if="errors.guard_name" class="text-red-500">{{
            errors.guard_name
          }}</span>
        </div>
      ...
      <div
        class="border rounded-4 flex flex-col gap-4 w-full h-full p-4 flex gap-6 overflow-hidden"
      >
        <div class="h-full grid grid-cols-3 gap-6 overflow-y-auto">
          ...
          >
            <label class="flex items-center gap-2" :for="'permission' + index"
              ><i class="i-hugeicons-stop"></i> {{ permission.name }}
            </label>
            <ToggleSwitch
              :id="'permission' + index"
